...
> [!NOTE]
> **本節は §23 で置き換えられた。** 差分の材料は `Item.coverVersion` ではなく、配布元に置かれた目録ファイルである。以下は、なぜその設計に至ったかの記録として残す。
...
> [!NOTE]
> **本節の「iCloud への再送信を伴う」という注意書きは §23 で無効になった。** 登録は蔵書レコードに触れないため、iCloud の通信は発生しない。
...
> [!NOTE]
> §23（目録方式）と §24（シャード名）、および §25（実ライブラリ規模への対応）で、本章の内容の一部は置き換えられている。
...
## 23. 配布元の目録方式への変更（2026-09-20）

### 23.1 やめたこと

§21.5 と §21.11 は、差分の判定に `Item.coverVersion` を使い、一括登録時に全件を 0 → 1 へ書き換える設計だった。これは **19,287 件のレコード更新を iCloud へ流し、他のすべての端末がそれを受信する**ことを意味する。実測では、受信側が 110% CPU を数分間使い続けた（内訳はすべて CloudKit の XPC デコードと SQLite への書き込み）。

**表紙の所在を知らせるためだけに、蔵書データベース全体をクラウド経由で往復させていた。** サムネイルは iCloud に送らないという判断をしておきながら、そのサムネイルの目録を iCloud に流していたことになる。
...
```
<配布元>/.shelfrow-thumbnails-index.json
{ "formatVersion": 1,
  "updatedAt": "...", "updatedBy": "...",
  "entries": { "<itemID>": { "version": 1, "bytes": 42133 }, ... } }
```

* 19,287 件で **約 1 MB**。受信側はこれを 1 回読むだけで、取得すべきものが分かる。
* **`Item.coverVersion` は配布経路から外れた。** 登録も取得も蔵書レコードに触れないため、**iCloud の通信はゼロ**になる。
* 差分の判定は「目録の version / bytes」と「この端末の `LocalCoverState`」の比較。送信対象は「ローカルにあるファイルのうち、目録に無いか、サイズが違うもの」。**サイズが違う = 表紙を選び直した**、という判定になる（ファイルは 1 回の描画から 1 回書かれるため）。
* 目録は**転送が終わってから**書く。届かなかったファイルの entry を先に載せると、他の端末が存在しないものを探しに行く。

### 23.3 副産物: 表示に応じた取得が速くなった

生成経路は ZIP を開く前に配布元を見るが、**共有越しでは「無い」と答えられるのも「ある」と答えられるのと同じ往復コストがかかる。** 目録があれば、その本が配布元に無いことをメモリ上の `Set<UUID>` で即座に判定でき、往復が発生しない。一括生成で 2 万回払っていた無駄な往復が消えた。

### 23.4 制約

読んで・混ぜて・書き戻す方式なので、2 台が同時に登録すると一方の entry が失われうる。1 人が端末を使い分ける前提（D1）では実害が小さく、失われた側は次回の周回で「自分のファイルが目録に無い」ことに気づいて載せ直す。

### 23.5 §21・§22 への影響

* §21.5 の差分の式、§21.11 の「iCloud への再送信を伴う」注意書き、§22.1 の該当行は本章で置き換える。
* §21.6 の役割の表は変わらない（一括登録は 1 台目のみ、単発の生成はどの端末でも）。

---

## 24. シャード名の大文字小文字（2026-09-20）

### 24.1 症状

実 NAS（SMB）での初回登録で、19,218 件中 **7,532 件だけが成功し、残り 11,686 件が全滅**した。ログは `11686 of 11686 transfers did not complete` とだけ言い、理由を残していなかった。

### 24.2 原因

配布元のシャードの分布を見ると、**空なのは 16 進の a〜f を含む 156 個**で、数字だけの 100 個にしか入っていなかった（100 × 約 75 件 ≒ 7,532）。

共有に対して直接確かめた結果:

```
mkdir 0A   →  "File exists"                 作成時は大文字小文字を区別しない
open 0A/…  →  "No such file or directory"   参照時は区別する
```

この NAS は**ディレクトリ名を作成時に小文字化して保存し、パスの解決は大文字小文字を区別する。** アプリは `UUID.uuidString`（大文字）から `0A` を作ろうとして「既にある」と言われ、その `0A` に書こうとして「無い」と言われていた。数字のシャードには大文字小文字が無いので成功していた。

ファイル名の方は大文字のまま保存されている（サーバはファイル名の綴りは保つ）。壊れていたのはディレクトリ名だけ。

### 24.3 対処

**シャード名を小文字にする。** サーバが保存する綴りと、アプリが参照する綴りを一致させる。既存の 7,532 件は数字のシャードにあり大文字小文字を持たないため、influenced されず、再送信も起きない。

ファイル名は `UUID.uuidString`（大文字）のまま変えない。サーバが保つ以上、変える理由がなく、変えれば既存の 7,532 件が見えなくなって再アップロードになる。

### 24.4 併せて直したこと

失敗時のログにエラーコードの内訳を出すようにした。`11686 of 11686 did not complete` だけでは、次に同じことが起きたときにまた共有を直接調べることになる。

---

## 25. 実ライブラリ規模への対応（2026-09-20）

### 25.1 症状

19,287 件のライブラリで、アプリの動作が目に見えて重くなった。起動直後から重く、配布の実行中はさらに重い。

### 25.2 原因（いずれも「規模を考えずに書いた」もの）

| 箇所 | 何をしていたか |
|---|---|
| `attach()` | 起動のたびに `adoptLocalFiles` / `forgetOrphanedStates` / `counts` の 3 パスが走り、**それぞれが蔵書全件を走査**していた。配布元を設定していない端末でも走る |
| 存在確認 | 1 冊ごとに `fileExists` / `attributesOfItem`。19,287 回のシステムコールを、1 回のディレクトリ列挙が持っている答えのために払っていた |
| fetch | `FetchDescriptor<Item>()` で**レコードを丸ごと** materialize |
| 転送 | ブロッキング I/O を `withTaskGroup` の中で実行。**協調スレッドプールのスレッドはコア数程度**しかなく、6 本の転送で塞ぐと、アプリ内の他のタスク（スクロールを含む）がその後ろに並ぶ |
| 保存 | 実行の最後に 1 トランザクションで全行を保存 |
| 生成経路 | 表紙を生成するたびに配布元へ問い合わせ。**共有越しでは「無い」と答えられるのも「ある」と答えられるのと同じ往復コスト**で、一括生成では 2 万回払っていた |
| 転送の実装 | `replaceItemAt`（旧ファイルの退避・メタデータの移送で往復が増える）、シャードディレクトリの作成をファイルごとに実行 |

### 25.3 対処

* **起動時は何もしない。** 配布元があり、かつ同期が有効なときだけ、起動 5 秒後の定期処理で行う。
* キャッシュの内容は **1 回のディレクトリ列挙**で `[UUID: Int]`（サイズ付き）にする。
* `propertiesToFetch` で `id` / `coverVersion` / `coverBytes` だけを取る。
* 転送は**専用の並行キュー**へ。協調スレッドプールを塞がない。
* 保存は 500 件ごと。
* 生成経路の問い合わせは、目録から作った `Set<UUID>` で**メモリ上で判定**（§23.3）。
* `replaceItemAt` をやめて `rename` に。ダウンロード側の二重のアトミック書き込みも解消。シャードの作成は 1 回だけ記憶する。

### 25.4 測定して分かった、私たちの負荷ではなかったもの

重さの訴えに対し、当初 Spotlight のインデックス作成を疑ったが、配布元フォルダは数日前から存在しており、説明として成り立たなかった。**推測をやめて測った結果**:

* ShelfRow を起動していない状態で `dasd` が 98.9% CPU。`appstoreagent` が `ArcadePayoutReset` を**毎分約 4,300 回**繰り返していた（次回実行時刻を、すでに過ぎた時刻に設定し直し続けるループ）。日付が変わった瞬間（00:00）に始まっており、ShelfRow とは無関係。
* ShelfRow 起動時の 110% CPU をサンプリングしたところ、**ほぼ全部が CloudKit**（XPC でのレコードのデコードと SQLite への書き込み）。サムネイル配布のコードは 1 回も動いていなかった。これが §23 の目録方式につながった。

### 25.5 `WAL checkpoint: Database busy` について

CoreData の **debug レベル**のメッセージで、エラーではない。「WAL のチェックポイントを試みたが、別の接続が DB を使っていたので後回しにする」という意味で、SQLite が後で再試行する。CloudKit のミラーリングが受信と送信を同時に行っている間に頻出する。出力を抑える公開 API は無く、競合そのものは正常な動作である。同期が落ち着けば止まる。

### 25.6 目録だけ残った配布ファイルの修復

初回の実NAS登録で失敗したファイルは、旧実装が先に目録へ載せていたため、目録上は存在するのに配布元には実ファイルが無い状態になった。取得側には `NSFileReadNoSuchFileError` が記録されるが、従来の登録判定は目録とローカルファイルのサイズだけを比較し、同じサイズなら登録済みと判断していた。このため1台目に元画像があっても修復できなかった。

取得失敗が記録されていてローカルファイルが存在する場合は、目録の版を増やさず同じ版で再登録する。ローカルファイルの採用時にも取得失敗を消さず、配布元への書き込みが成功した時点でのみ失敗状態を解除する。これにより「Stackroomサムネイルの移行」で旧資産をキャッシュへ戻した後、「サムネイルを配布元へ登録」を実行すれば、目録と実ファイルの食い違いを修復できる。
