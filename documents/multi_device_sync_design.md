...
- FR3 スイッチの ON/OFF を再起動なしで切り替えられる（フォールバックとして再起動方式を許容、§6.5）。
...
### 5.4 ホットスワップの前提とリスク
- 同一 SQLite ファイルを旧コンテナが閉じる前に新コンテナが開く瞬間が生じうる。Core Data は複数コーディネータからの同一ファイル同時オープンを許容するが、CloudKit ミラーリングが動いている側とローカル側が同時に書く状況は避けたい。対策として、手順 4 の前に旧コンテナの `mainContext` を `save()` し、ビューを先に破棄（`generation` を先に更新して空のプレースホルダビューを一瞬表示）してから新コンテナを開く。
- これでも不安定な場合は §6.5 の再起動方式へフォールバックする。設計上どちらでも `LibraryStore` の外側は変わらない。
...
**問題**: Mac A と Mac B がそれぞれローカルモードで同じ Stackroom XML をインポート済み → A が先に cloud へ → B も cloud へ、という順序では、B の初回エクスポートで同じ本が二重にクラウドへ登録される（UUID はインポート時に端末ごとに別々に振られるため）。

**判定**: cloud へ切り替えて初期インポートが完了した時点で、
- インポート前のローカル件数 `L > 0` かつ、インポートで流入した件数 `C > 0` なら「両側非空」。

**選択ダイアログ**:
1. **クラウドを正にする（推奨）**: ローカル由来のレコードを削除し、クラウド由来のみ残す。サムネイルキャッシュ・ブックマークは `LocalBookmark` / `LocalCoverState` が `targetID` で紐づくため、`legacyID` または（`relativePath`, `volume`）が一致するクラウド側レコードへ**付け替えて**残す。
2. **ローカルを統合する**: `legacyID`（無ければ `volume.id` + `relativePath`）で重複を検出し、重複はクラウド側を残してローカル側を削除。重複しないローカルレコードのみエクスポートする。
3. **キャンセル**: スイッチを OFF に戻し、ローカルはそのまま。

初期インポート完了前にダイアログを出すと件数が確定しないため、§7.4 の条件を待つ。待機中は状態欄に「初回同期中… N 件」を表示する。
...
| Q3 | ホットスワップの安定性 | Phase 1 で実装・検証。不安定なら再起動方式（§6.5） |
